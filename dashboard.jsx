...
- Never mention specific muscle groups or body parts in any field`;
  ...
  async function generateWeeklyReview(forceRefresh=false) {
    ...
    try {
      ...
      const now = new Date();
      const _lps = cycleLog?.last_period_start || null;
      let cyclePhaseStr = null;
      if(_lps){const {phase}=calculateCyclePhase(_lps,cycleLog?.avg_cycle_length||28);cyclePhaseStr=phase;}
      const systemPrompt = buildCoachSystemPrompt(profileData,{cyclePhase:cyclePhaseStr?`Cycle phase: ${cyclePhaseStr}`:null},profileData?.detected_patterns||[],profileData?.behavioral_baseline||null,{trainingDays:weekWorkouts.length,proteinDaysHit:protDaysHit,stepDaysHit:stepDaysHit,avgSleep:avgSleepMin?`${Math.floor(avgSleepMin/60)}h${avgSleepMin%60}m`:"n/a",pendingFeedback:[]});
  ...
  return (
    <div>
      ...
      {(()=>{
        ...
        const sleepThisWeek=(fitbitData.sleep||[]).filter(s=>weekKeys.includes(s.date));
        const avgSleepMin=sleepThisWeek.length?Math.round(sleepThisWeek.reduce((a,r)=>a+r.total,0)/sleepThisWeek.length):null;
        const protDays=weekKeys.filter(dk=>(allFood[dk]||[]).reduce((s,e)=>s+(e.p||0),0)>=(profileData?.protein_target||100)*0.9).length;
        const rows=[
          ...
          <div style={s.aiCard}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 12px",marginBottom:6}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:12}}>
                <span style={{color:C.t2}}>Avg sleep</span>
                <span style={{fontWeight:600,color:C.tx}}>{avgSleepMin?`${Math.floor(avgSleepMin/60)}h${avgSleepMin%60}m`:"—"}</span>
              </div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:12}}>
                <span style={{color:C.t2}}>Protein days</span>
                <span style={{fontWeight:600,color:protDays>=5?C.teal:C.tx}}>{protDays}<span style={{fontWeight:400,color:C.t3}}>/7</span></span>
              </div>
            </div>
          </div>
        );
