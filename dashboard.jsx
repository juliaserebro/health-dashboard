...
TRAINING SUGGESTION RULES:
- daysSinceLastWorkout === 0 (trained today): acknowledge the session — do NOT suggest training again today
- daysSinceLastWorkout === 1: acknowledge yesterday's session; suggest a complementary category (e.g. if yesterday was strength → suggest mobility or cardio next; never same category two days running unless adherence and targets support it)
- daysSinceLastWorkout >= 2: encourage activity; suggest the category most behind on weekly target
- NEVER say "looks like a good day to train" without naming which category and exactly why that category
...
Return ONLY valid JSON, nothing else:
{
  ...
  "nutrition": "1 sentence. Based ONLY on actual food logged today. ALREADY EATEN TODAY — these are forbidden from suggestion: ${foodSummary||'nothing yet'}. You MUST NOT mention, suggest, or reference any item from that list. Protein so far: ${prot}g of ${protTgt}g target. If protein is close to or at target, say so. Otherwise suggest a specific food NOT in the eaten list.",
...
Rules:
- Decide overall_signal first — headline and all subsections must reflect it
  ...
  async function refreshNutritionOnly() {
    ...
    try {
      const raw = await callCoachAI(
        `Generate ONLY the nutrition insight for today. One sentence.
ALREADY EATEN TODAY (forbidden from suggestion): ${foodSummary2}
Protein so far: ${prot}g of ${protTgt2}g target.
Rules: do not suggest any food already eaten; if protein is close to or at target acknowledge it positively; if nothing logged say so neutrally; if genuinely nothing new to say return the single word: null`,
      ...
      if(txt && txt.toLowerCase()!=="null"){
        const updated = {...coachContent, nutrition:txt, _foodHash:currentFoodHash, _generatedAt:new Date().toISOString()};
        setCoachContent(updated);
        localStorage.setItem("coach_content_"+todayKey, JSON.stringify(updated));
      } else {
        // Nothing new to say — just update the hash so we don't keep checking
        const updated = {...coachContent, _foodHash:currentFoodHash};
        setCoachContent(updated);
        localStorage.setItem("coach_content_"+todayKey, JSON.stringify(updated));
      }
...
🌙 SLEEP PATTERN: **Your deep sleep averaged X minutes.** This correlates with Y.`;
  ...
  useEffect(()=>{ if(apiKey && profileData && fitbitReady && todayFoodCount>0) refreshNutritionOnly(); },[todayFoodCount]);
      ...
      {apiKey&&(
        <div style={s.aiCard}>
          ...
          {(()=>{
            ...
            const hourNow=parseInt(new Date().toLocaleString("en-CA",{timeZone:getTz(),hour:"numeric",hour12:false}));
            const subsections = [
              {key:"recovery", label:"💪 Recovery"},
              ...(hourNow>=19?[{key:"tonight",label:"🌙 Tonight"}]:[]),
...
// ── FOOD TAB ──────────────────────────────────────────────────────────────
function TabFood({allFood, setAllFood, protTgt, apiKey, onFoodLogged, suppState={}, setSupp, profileData, onSaveSupps}) {
