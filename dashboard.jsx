...
const UID = "00000000-0000-0000-0000-000000000001"; // stable UUID — works with both text and uuid column types
...
const GOAL_SUBS = {
  ...
  more_energy:{label:"More energy & recovery",prompt:"How do you want to measure progress?",options:[
    ...
    {id:"both",label:"Both energy and recovery"},
  ]},
  ...
  return (
    <div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        {supps.map((sup,i)=>{
          const query=suppDropdowns[i];
          const suggestions=query?SUPPLEMENT_LIST.filter(s=>s.toLowerCase().includes(query.toLowerCase())).slice(0,6):[];
          return (
            <div key={i} style={{display:"flex",gap:8,marginBottom:8,alignItems:"flex-end"}}>
              <div style={{flex:2,position:"relative"}}><label style={lbl}>Name</label>
                <input type="text" value={sup.name||""} onChange={e=>{
                  const v=e.target.value;
                  setSupps(arr=>arr.map((x,j)=>j===i?{...x,name:v}:x));
                  setSuppDropdowns(d=>({...d,[i]:v}));
                }} onBlur={()=>setTimeout(()=>setSuppDropdowns(d=>({...d,[i]:""})),150)} style={s.input}/>
                {suggestions.length>0&&suppDropdowns[i]&&(
                  <div style={{position:"absolute",top:"100%",left:0,right:0,background:C.sf,border:`.5px solid ${C.bd}`,borderRadius:8,maxHeight:200,overflowY:"auto",zIndex:20,boxShadow:"0 4px 12px rgba(0,0,0,.1)"}}>
                    {suggestions.map(s=>(
                      <div key={s} onMouseDown={()=>{
                        setSupps(arr=>arr.map((x,j)=>j===i?{...x,name:s}:x));
                        setSuppDropdowns(d=>({...d,[i]:""}));
                      }} style={{padding:"6px 10px",cursor:"pointer",fontSize:13}} onMouseEnter={e=>e.target.style.background=C.s2} onMouseLeave={e=>e.target.style.background=""}>{s}</div>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div style={{flex:2}}><label style={lbl}>Timing</label>
                <select value={sup.timing||""} onChange={e=>setSupps(arr=>arr.map((x,j)=>j===i?{...x,timing:e.target.value}:x))} style={s.input}>
      ...
      <Card style={{marginBottom:14}}>
        ...
        {editTargets ? (
          <>
            {[
              ...
              <div key={k} style={{marginBottom:14}}>
                <div style={{fontWeight:600,color,fontSize:13,marginBottom:3}}>{label}</div>
                <div style={{fontSize:11,color:C.t2,lineHeight:1.6,marginBottom:2}}>{desc}</div>
                <div style={{fontSize:10,color:C.t3,marginBottom:6}}>Examples: {examples}</div>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <input type="number" min="1" max="5" value={targets[k]??1} onChange={e=>setTargets(t=>({...t,[k]:parseInt(e.target.value)||1}))} style={{...s.input,width:90}}/>
                  <span style={{fontSize:11,color:C.t3}}>sessions/week</span>
                </div>
              </div>
            ))}
            {(()=>{
              ...
              return (
                <>
                  <div style={{fontSize:12,color:total===5?C.teal:C.am,marginBottom:4,fontWeight:500}}>Total: {total} sessions / week · {Math.round(total*4.3)} sessions / month{total!==5?" ⚠ aim for 5 total":""}</div>
          ...
          <>
            ...
            <div style={{fontSize:11,color:C.t3,marginTop:4,marginBottom:8}}>Total: {(targets.strength||0)+(targets.mobility||0)+(targets.cardio||0)} sessions/week · {Math.round(((targets.strength||0)+(targets.mobility||0)+(targets.cardio||0))*4.3)}/month</div>
      ...
      {showSett&&!isViewOnly&&(
        <div style={s.mo} onClick={e=>{if(e.target===e.currentTarget)setShowSett(false);}}>
          <div style={{...s.modal,width:420}}>
            ...
            <p style={{fontSize:11,color:C.t3,marginBottom:16}}>Stored locally in your browser only.</p>
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
