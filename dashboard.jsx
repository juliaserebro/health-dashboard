    ...
    <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            {[["BMI",bmi.toFixed(1)],["BMR",Math.round(bmr)+" kcal/day"]].map(([l,v])=>(
              <div key={l} style={{background:C.s2,borderRadius:8,padding:12}}>
                ...
                <div style={{fontSize:10,color:C.t3,marginTop:2}}>{l==="BMI"?"calculated from height & weight":"basal metabolic rate (Mifflin-St Jeor)"}</div>
              </div>
            ))}
          </div>
        );
      ...
      <Card style={{marginBottom:14}}>
        ...
        {[0,1,2].map(slot=>{
          ...
          return (
            <div key={slot} style={{marginBottom:10,paddingBottom:10,borderBottom:slot<2?`.5px solid ${C.bd}`:"none"}}>
              <div style={{fontSize:11,color:C.t3,marginBottom:4}}>Goal {slot+1}</div>
              <select value={g.id||""} onChange={e=>{
                const id=e.target.value; const o=GOAL_OPTIONS.find(x=>x.id===id);
                setGoals(arr=>{const next=[...arr]; next[slot]=id?{id,label:o?o.label:id,definition:null}:null; return next;});
              ...
              {opt&&(
                <div style={{paddingLeft:12}}>
                  <div style={{fontSize:11,color:C.t3,marginBottom:3}}>What does this mean for you?</div>
                  <select value={g.definition||""} onChange={e=>setGoals(arr=>{const next=[...arr]; next[slot]={...next[slot],definition:e.target.value}; return next;})} style={{...s.input,width:"100%"}}>
      ...
      <Card style={{marginBottom:14}}>
        ...
        {[["strength","Strength"],["mobility","Mobility"],["cardio","Cardio"]].map(([k,l])=>(
          <div key={k} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            ...
            <span style={{fontSize:11,color:C.t3,width:70}}>≈ {Math.round((targets[k]??0)*4.3)}/month</span>
          </div>
        ))}
        <div style={saveRow}>
      ...
      {/* Activity mapping — only show types that need user decision */}
      {(()=>{
        const needsDecision = seenTypes.filter(t=>getActivityCategory(t,{})==="uncategorized"&&!mapping[t]);
        const userOverrides = seenTypes.filter(t=>mapping[t]&&mapping[t]!==DEFAULT_ACTIVITY_MAPPING[t]);
        if(needsDecision.length===0&&userOverrides.length===0) return null;
        return (
          <Card style={{marginBottom:14}}>
