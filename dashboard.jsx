...
// The gate is deterministic and free. A checkpoint that fires only to say
// "you're doing fine" is a notification tax, so it stays silent when nothing
// is off - and costs nothing when it does.
function checkpointGate(kind, m, tgt){
  if(m.meals_today===0) return {say:true, reason:"nothing logged yet today"};
...
- PARTIAL REGENERATION: this is an update, not a fresh morning. Return the previous insights that the event does not touch UNCHANGED, word for word, and rewrite only the ones the event actually affects. Rewriting a still-true observation makes the card look unstable for no reason. If the event affects nothing, return the previous content unchanged.`;
  ...
  async function runFoodCheckpoint(kind){
    ...
    const gate = checkpointGate(kind, m, tgt);
    ...
    const sys = `You are Julia's nutrition coach, checking in during the day. Reply with ONE short sentence, or the single word null if there is genuinely nothing useful to say.
...
Write her running protein as the placeholder {protein_today} - never a literal number. The app substitutes the live value and adds units.`;
...
Suggest at most one concrete, easy thing she could still eat today.`;
    try{
      const raw = await callCoachAI(usr, sys, {model:MODEL_FAST, maxTokens:120, cache:false});
      const txt = raw.replace(/```/g,"").trim();
      if(!txt || txt.toLowerCase()==="null"){ console.log("Coach checkpoint ("+kind+"): model had nothing to add."); return; }
