...
// ── CALENDAR-DAY HELPERS (date parts, active timezone — never 24h-block math) ──
function todayKeyTz(){ return new Date().toLocaleDateString("en-CA",{timeZone:getTz()}); }
function dayKeyToNoon(dk){ const [y,m,d]=dk.split("-").map(Number); return new Date(y,m-1,d,12); }
// Whole calendar days between a YYYY-MM-DD key and today (0 = today, 1 = yesterday)
function calDaysAgo(dk){ return Math.round((dayKeyToNoon(todayKeyTz())-dayKeyToNoon(dk))/864e5); }
// Calendar date key (in the active tz) for a full ISO timestamp
function tsToDayKey(ts){ return new Date(ts).toLocaleDateString("en-CA",{timeZone:getTz()}); }
// The uniform temporal label every prompt uses: [2026-08-12 | Wednesday | 18 days ago]
function tripleLabel(dk){
  const n=calDaysAgo(dk);
  const wd=dayKeyToNoon(dk).toLocaleDateString("en-GB",{weekday:"long"});
  const rel=n<=0?"today":n===1?"yesterday":`${n} days ago`;
  return `[${dk} | ${wd} | ${rel}]`;
}
// Recency bucket, honouring the configured week start (Sunday by default)
function recencyBucket(dk){
  const n=calDaysAgo(dk);
  if(n<=0) return "TODAY";
  if(n===1) return "YESTERDAY";
  const [y,m,d]=todayKeyTz().split("-").map(Number);
  const intoWeek=daysSinceWeekStart(new Date(y,m-1,d).getDay());
  if(n<=intoWeek) return "EARLIER THIS WEEK";
  if(n<=intoWeek+7) return "LAST WEEK";
  return "OLDER";
}
// Day key N calendar days before today — built from date parts (DST-safe),
// replacing repeated −86,400,000 ms stepping
function dayKeyNBack(n){
  const [y,m,d]=todayKeyTz().split("-").map(Number);
...
async function saveCycleDates(newDate, avgPeriodLen=5) {
  ...
  // Phase 0: store ALL period start dates indefinitely — write-time truncation
  // was permanently deleting the 7th-oldest date on every save. Any capping
...
TODAY IS ${todayKeyTz()} (${dayKeyToNoon(todayKeyTz()).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}).
DATE LABEL RULE: dated items in this conversation carry precomputed labels [date | weekday | N days ago]. Use them VERBATIM when referring to timing. NEVER characterise recency yourself — no "last week", "recently", "a few days ago", or "your update from last week" unless it matches the supplied label exactly.
...
Health notes: ${profileData?.health_notes||'none'}${(todayData&&todayData.recoveryBlock)||''}${(todayData&&todayData.conditionsBlock)||''}
...
async function runPatternDetection(profileData, fitbitData, allFood, cycleDates, cycleLog, logEntries) {
  ...
  if(profileData?.cycle_tracking && ACTIVE_CYCLE_TRACKING){
    ...
    (logEntries||[]).forEach(e=>{
      ...
      // Structured Life Context taps are the primary, machine-checkable signal
      if(contextTagsOf(e).length) confounded.add(dk);
      // Free-text fallback for older entries and other categories
      const t=(e.txt||"").toLowerCase();
      if(/wine|alcohol|beer|drink|cocktail/.test(t)) confounded.add(dk);
      if(/\bill\b|sick|flu|fever|cold|travel|flight|jet ?lag|stress|injur/.test(t)) confounded.add(dk);
    });
...
// "Ongoing" and an event-bounded option would behave identically -- no expiry,
// no prompt, user-dismissed only -- and two labels with one behaviour is a
// choice with no consequence. Keeping the honest one: "Ongoing" implies a
// certainty the user often does not have, and for surgery, test results or an
// unresolved injury, "I don't know" is simply true.
//
// null span = never expires, never prompts, lives on Today until dismissed.
const DURATIONS = [
  ["today",   "Just today",   1],
  ["days",    "A few days",   4],
  ["weeks",   "A few weeks",  21],
  ["unknown", "I don't know", null],
];
// Legacy rows written before the rename still resolve.
const DURATION_ALIASES = {ongoing:"unknown"};
const DURATION_LABEL = (k)=>(DURATIONS.find(d=>d[0]===(DURATION_ALIASES[k]||k))||[,"Just today"])[1];
...
function expectedEndOf(cond){
  ...
  const est = DURATION_ALIASES[cond.duration_est] || cond.duration_est || "today";
  if(est==="unknown") return null;                       // never expires
  if(cond.expected_end !== undefined && cond.expected_end !== null) return cond.expected_end;
  const row = DURATIONS.find(d=>d[0]===est);
...
// ── SCHEDULED-ARTIFACT VISIBILITY ───────────────────────────────────────────
// The weekly review stopped firing and nobody noticed for five weeks. It was
// found by chance, reading a timestamp during an unrelated check. Several
// things here are supposed to happen on a schedule and there was no way to tell
// whether any of them were working short of querying the database by hand.
//
// This prints one table on load. Not user-facing; it exists so "silently
// stopped" is visible the next time it happens, to whichever of us looks first.
function reportScheduledArtifacts(profileData){
  if(!profileData) return;
  const now = Date.now();
  const ageOf = (ts)=>{
    if(!ts) return null;
    const t = new Date(ts).getTime();
    return isNaN(t) ? null : Math.floor((now - t)/864e5);
  };
  ...
  const out = rows.map(([name, ts, staleAfter])=>{
    const age = ageOf(ts);
    const status = ts===null ? "NEVER RECORDED"
      : age===null ? "unparseable"
      : age>staleAfter ? `STALE (${age}d, expected <=${staleAfter}d)`
      : `ok (${age}d)`;
    return {artifact:name, generated: ts ? String(ts).slice(0,16).replace("T"," ") : "-", status};
  });
  const bad = out.filter(r=>/STALE|NEVER/.test(r.status));
  console.log("Health Coach - scheduled artifacts" + (bad.length?`  (${bad.length} need attention)`:"  (all current)"));
  if(console.table) console.table(out); else out.forEach(r=>console.log("  ",r.artifact,r.generated,r.status));
}

// ── PHASE 5.4 MIGRATION ─────────────────────────────────────────────────────
// This was specified and never written: the conditions column stayed empty, so
// a spinal surgery logged on 18 Aug lived on only as a life_context journal
// entry -- and would have dropped out of the coach's 14-day log window the very
// next day.
//
// Anything that reads as a procedure or recovery becomes an ACTIVE condition
// with duration "unknown": never expiring, never prompting, dismissed only by
// the user. It must never default to resolved -- a surgery is not over because
// the app could not guess an end date.
// Other historical life_context entries carry no duration and are recorded as
// already ended, per the original brief.
const MIGRATION_KEY = "conditions_migrated_v1";
function buildConditionsFromLog(logEntries, existing){
  const have = new Set((existing||[]).map(c=>String(c.id)));
  const out = [];
  (logEntries||[]).forEach(e=>{
    if(!e || !e.dt || have.has(String(e.id))) return;
    const tag = e.tag||"";
    if(tag!=="life_context" && tag!=="life" && tag!=="medical") return;
    const startKey = tsToDayKey(e.dt);
    const recovery = isRecoveryText(e.txt);
    const cond = {
      id: e.id,
      text: String(e.txt||"").replace(/\s+/g," ").trim().slice(0, 180),
      start_date: startKey,
      duration_est: recovery ? "unknown" : "days",
      expected_end: null,
      extensions: [],
      // Non-recovery history is recorded as already ended (brief 5.4). Recovery
      // stays OPEN. resolved_at is the migration's only judgement, and it errs
      // toward keeping something active rather than silently closing it.
      resolved_at: recovery ? null : startKey,
      migrated: true
    };
    if(!recovery) cond.expected_end = startKey;
    out.push(cond);
  });
  return out;
}

// ── RECOVERY / POST-OPERATIVE GATE ──────────────────────────────────────────
// Health notes said "CLEARED BY DOCTOR FOR ALL PHYSICAL ACTIVITY" for two weeks
// after spinal surgery, and the coach kept recommending strength work. A user
// editing a text field is not a safety mechanism: anyone's notes can go stale.
// So this gate reads STRUCTURE (an active condition), with a prose fallback for
// entries not yet migrated -- the same shape as the pain gate, for the same
// reason.
//
// It blocks PRESCRIPTION, not conversation. Recovery involves prescribed
// movement (walking, physiotherapy), and an app that refuses to acknowledge any
// of it is unhelpful in the other direction. The app does not prescribe; it
// responds.
const RECOVERY_RE = /\b(surger\w*|surgical|operation|post[-\s]?op\w*|recover\w*|rehab\w*|physio\w*|discharg\w*|stitches|sutures|incision|fracture[ds]?|broken\s+\w+|in\s+a\s+cast|immobilis\w*|immobiliz\w*|not\s+allowed\s+to\s+lift|no\s+lifting|bed\s?rest)\b/i;
function isRecoveryText(t){ return RECOVERY_RE.test(String(t||"")); }
...
function recoveryActiveOn(conditions, logEntries, dayKey, healthNotes){
  ...
  const fromLog = (logEntries||[]).filter(e=>{
    if(!e || !e.dt || !isRecoveryText(e.txt)) return false;
    const k = tsToDayKey(e.dt);
    return k <= dayKey;   // deliberately NOT age-limited: see the 14-day note
  });
  // Most recent first; one is enough to gate.
  fromLog.sort((a,b)=>String(b.dt).localeCompare(String(a.dt)));
  return fromLog.slice(0,2).map(e=>({text:e.txt, since:tsToDayKey(e.dt)}));
}
...
(+${omitted.length} older ${omitted.length===1?"entry":"entries"} omitted to save space: ${parts.join(", ")}. Nothing pain- or workout-related was omitted — the record above is complete for those.)`;
  ...
  const buckets={};
  entries.forEach(e=>{
    const b=recencyBucket(e.dk);
    (buckets[b]=buckets[b]||[]).push(`${tripleLabel(e.dk)} [${e.tag}] ${e.txt}`);
  });
...
function buildCtxFull({allFood, logEntries, cycleDates, cycleLog, protTgt, fitbitData, profileData}) {
  ...
  const recentWorkouts=[...(fitbitData.workouts||[])].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,5).map(w=>tripleLabel(w.date)+" "+w.type+(w.duration_min?" "+w.duration_min+"min":"")).join(", ");
...
// Model tiering (C.1/C.2). MAIN reasons over the full context; FAST handles
// narrow single-fact tasks whose context is a handful of numbers.
// NOTE: Haiku 4.5's API id is the dated form - `claude-haiku-4-5` is only an
// alias. Do not "modernise" it.
const MODEL_MAIN = "claude-sonnet-5";
const MODEL_FAST = "claude-haiku-4-5-20251001";

const COACH_SCHEMA = 3; // 3 = running totals are {placeholders}, not literals

// ── FOOD SENSITIVITY ENFORCEMENT ────────────────────────────────────────────
// Measured, not assumed: with "NEVER suggest anything conflicting with: dairy,
// eggs, whey" in the prompt, Haiku violated it 3/3 and Sonnet 2/3 - one reply
// named a whey shake outright. A constraint that can be enforced in code must
// never be left to the prompt. The prompt still states the rule (it improves
// the odds); this filter is what actually guarantees it.
const SENSITIVITY_TERMS = {
  dairy:["dairy","milk","cheese","yoghurt","yogurt","cottage cheese","cream","butter","kefir","whey","casein","labneh","ricotta","mozzarella","feta","paneer","skyr","quark"],
  lactose:["dairy","milk","cheese","yoghurt","yogurt","cream","butter","kefir","ice cream"],
  egg:["egg","eggs","omelette","omelet","frittata","shakshuka","mayonnaise","meringue","quiche"],
  whey:["whey","protein shake","protein powder","protein smoothie"],
  gluten:["gluten","bread","pasta","wheat","couscous","cracker","crackers","pita","bagel","barley","rye","seitan","tortilla","wrap","noodles"],
  wheat:["wheat","bread","pasta","couscous","cracker","crackers","pita","bagel","tortilla","wrap"],
  nut:["nut","nuts","almond","almonds","walnut","walnuts","cashew","cashews","pistachio","pistachios","pecan","pecans","hazelnut","hazelnuts","peanut","peanuts","nut butter","almond butter","peanut butter"],
  peanut:["peanut","peanuts","peanut butter"],
  soy:["soy","soya","tofu","edamame","tempeh","miso","soy sauce"],
  fish:["fish","salmon","tuna","cod","sardine","sardines","anchovy","anchovies","mackerel","trout","herring"],
  shellfish:["shellfish","shrimp","prawn","prawns","crab","lobster","mussel","mussels","oyster","oysters","calamari","squid"],
  pork:["pork","bacon","ham","prosciutto","chorizo","salami","pancetta"],
  "red meat":["beef","steak","lamb","mince","burger","veal"],
  meat:["beef","steak","lamb","chicken","turkey","pork","mince","burger","bacon","ham"],
  caffeine:["coffee","espresso","caffeine","black tea","green tea","matcha","cola"],
  alcohol:["wine","beer","alcohol","spirits","whisky","vodka","gin"],
  sesame:["sesame","tahini","halva","hummus"],
  legume:["lentil","lentils","chickpea","chickpeas","bean","beans","hummus","legume","legumes"],
  nightshade:["tomato","tomatoes","pepper","peppers","aubergine","eggplant","potato","potatoes"],
  onion:["onion","onions","shallot","leek"],
  garlic:["garlic"],
  vegetarian:["beef","steak","lamb","chicken","turkey","pork","fish","salmon","tuna","bacon","ham","meat"],
  vegan:["beef","steak","lamb","chicken","turkey","pork","fish","salmon","tuna","bacon","ham","meat","dairy","milk","cheese","yoghurt","yogurt","egg","eggs","butter","whey","honey"]
};
// Everything she typed counts literally, plus anything a known category implies.
function sensitivityTerms(list){
  const out=new Set();
  (list||[]).forEach(raw=>{
    const s=String(raw||"").toLowerCase().trim();
    if(!s) return;
    s.split(/[,/]|\bor\b|\band\b/).map(x=>x.trim()).filter(Boolean).forEach(x=>out.add(x));
    Object.keys(SENSITIVITY_TERMS).forEach(k=>{
      if(s.includes(k)) SENSITIVITY_TERMS[k].forEach(t=>out.add(t));
    });
  });
  return [...out];
}
// Whole-word matching, so "nutrition" is not a nut and "creamy" is not cream.
function violatesSensitivities(text, list){
  const t = " " + String(text||"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim() + " ";
  return sensitivityTerms(list).filter(term=>{
    const tt = " " + String(term).toLowerCase().replace(/[^a-z0-9]+/g," ").trim() + " ";
    return tt.trim() && t.includes(tt);
  });
}

// B.8 - what the coach's content is genuinely a function of. If any of this
// changed, the time-based cache is stale no matter what the clock says; the
...
// call: the numbers are already on the device, and a confirmation that needs a
// network round-trip is a confirmation that sometimes never arrives.
function foodAck(entry, dayEntries, protTgt){
  const p = Math.round((dayEntries||[]).reduce((s,e)=>s+(e.p||0),0));
  const left = Math.max(0, Math.round((protTgt||110) - p));
  const name = entry&&entry.n ? entry.n : "Meal";
  if(left===0) return name+" logged ✓ · "+p+"g protein — target hit 🎯";
  return name+" logged ✓ · "+p+"g protein today, "+left+"g to go";
}

// -- B.7 FOOD CHECKPOINTS ---------------------------------------------------
// Her own meal rhythm, not a clock imposed on her: the median hour she actually
// eats lunch / dinner, over whatever history exists.
function usualMealHour(allFood, which){
  const hrs=[];
  Object.keys(allFood||{}).sort().slice(-30).forEach(d=>(allFood[d]||[]).forEach(e=>{
    const t=e.eaten_time||e.time; if(!t) return;
    const h=parseInt(String(t).slice(0,2),10), mi=parseInt(String(t).slice(3,5),10)||0;
    if(isNaN(h)) return;
    const dec=h+mi/60;
    if(which==="lunch" && h>=11 && h<=15) hrs.push(dec);
    if(which==="dinner" && h>=17 && h<=22) hrs.push(dec);
  }));
  if(hrs.length<3) return which==="lunch" ? 13 : 19.5;
  hrs.sort((a,b)=>a-b);
  return hrs[Math.floor(hrs.length/2)];
}
// Has she normally logged something by this hour? Distinguishes "a gap in an
// ordinary day" from "this is simply when she eats". Without it, someone who
// habitually skips lunch gets nudged about it every single day.
function typicallyEatenBy(allFood, hourDec){
  const days = Object.keys(allFood||{}).sort().slice(-14).filter(d=>(allFood[d]||[]).length>0);
  if(days.length < 5) return true; // too little history to justify silence
  const hits = days.filter(d=>(allFood[d]||[]).some(e=>{
    const t=e.eaten_time||e.time; if(!t) return false;
    const h=parseInt(String(t).slice(0,2),10), mi=parseInt(String(t).slice(3,5),10)||0;
    return !isNaN(h) && (h+mi/60) < hourDec;
  })).length;
  return hits/days.length >= 0.5;
}
// The gate is deterministic, free, and now the SOLE arbiter of silence: the
// prompt's "reply null if there's nothing to say" escape hatch was measured at
// 0/36 across both models, so it was decorative and has been removed. Every
// decision to stay quiet is made here, in code, before anything is spent.
function checkpointGate(kind, m, tgt, ctx){
  ctx = ctx || {};
  if(m.meals_today===0){
    // Nothing logged is only informative if she normally HAS logged by now.
    if(ctx.typicalByNow===false)
      return {say:false, reason:"nothing logged, but that is her usual rhythm at this hour"};
    return {say:true, reason:"nothing logged yet today"};
  }
  const pace = kind==="midday" ? 0.40 : 0.65;
  if(m.protein_today < tgt*pace*0.8)
    return {say:true, reason:"protein is behind pace for this time of day ("+Math.round(m.protein_today)+"g of a "+tgt+"g target)"};
  return {say:false};
}
...
function fillPlaceholders(text, m){
  return String(text||"").replace(METRIC_RE,(full,key,trailingUnit)=>{
...
function TabDash({allFood, logEntries, cycleDates, cycleLog, apiKey, protTgt, aiRefreshTick=0, fitbitData={sleep:[],steps:[],workouts:[]}, profileData=null, setProfileData}) {
  ...
  // opts: {model, maxTokens, cache}
  // C.3 - narrow tasks pass their own trimmed system prompt and a small budget
  //       rather than paying for the full 3k-token coaching context.
  // C.5 - the full system prompt is the large, stable half of every request and
  //       is byte-identical across a day's calls, so it is marked cacheable.
  //       Cache writes cost 1.25x and reads 0.1x, so this only pays off above
  //       the minimum cacheable size - hence the length gate.
  async function callCoachAI(userMessage, systemPrompt, opts={}) {
    const model = opts.model || MODEL_MAIN;
    const cacheable = opts.cache !== false && String(systemPrompt||"").length > 5000;
  ...
  // today, not a standing preference.
  const _dismissKey = "coach_dismissed_"+new Date().toLocaleDateString("en-CA",{timeZone:getTz()});
  const [dismissedTypes, setDismissedTypes] = useState(()=>{
    try{ return JSON.parse(localStorage.getItem(_dismissKey)||"[]"); }catch(e){ return []; }
  });
  function dismissInsight(type){
    setDismissedTypes(prev=>{
      const next = prev.includes(type) ? prev : [...prev, type];
      try{ localStorage.setItem(_dismissKey, JSON.stringify(next)); }catch(e){}
      return next;
    });
  }
  const [regenMsg, setRegenMsg] = useState("");
  const MANUAL_MAX_PER_DAY = 3, MANUAL_MIN_GAP_MIN = 20;
  function manualRegenerate(){
    const dayKey = "coach_manual_"+new Date().toLocaleDateString("en-CA",{timeZone:getTz()});
    let rec={n:0,last:0}; try{ rec=JSON.parse(localStorage.getItem(dayKey)||'{"n":0,"last":0}'); }catch(e){}
    const gapMin = (Date.now()-(rec.last||0))/60000;
    if(rec.n >= MANUAL_MAX_PER_DAY){ setRegenMsg("Refreshed "+MANUAL_MAX_PER_DAY+" times today — it updates on its own from here."); return; }
    if(gapMin < MANUAL_MIN_GAP_MIN){ setRegenMsg("Just refreshed — try again in "+Math.ceil(MANUAL_MIN_GAP_MIN-gapMin)+" min."); return; }
    try{ localStorage.setItem(dayKey, JSON.stringify({n:(rec.n||0)+1, last:Date.now()})); }catch(e){}
    setRegenMsg("");
  ...
  async function generateAllCoachContent(forceRefresh=false, triggeringEvent=null) {
    ...
    try{
      ...
      const last14Keys = Array.from({length:14},(_,i)=>dayKeyNBack(i)); // date-parts, DST-safe
      ...
      // Calendar days (date parts) — raw-now vs noon-anchor arithmetic made a
      // workout done yesterday read as "0 days ago" before local noon, so the
      // coach believed a session had already happened today.
      const daysSinceLastWorkout=lastWorkoutDate?calDaysAgo(lastWorkoutDate):99;
      const at = profileData?.activity_targets||{};
      ...
      // Saturday, not Sunday. The Israeli week runs Sunday-Saturday, so a
      // retrospective belongs at the END of it -- and the weekly review already
      // used Saturday, so the two disagreed about what "weekly" meant.
      // Verified nothing else depends on the Sunday assumption: isWeekly only
      // sets a display mode and a cache-validity flag on the coach card.
      const isWeekly=dow===6&&hour>=18;
...
⚠️ RECOVERY / POST-OPERATIVE — ACTIVE: ${_recovery.map(r=>`${String(r.text).slice(0,160)} (since ${r.since})`).join(" | ")}
ABSOLUTE RULE while this is active, regardless of anything in the health notes above:
- Do NOT recommend, prescribe, suggest or encourage ANY training, workout, session or exercise.
- Do NOT say it is a good day to train, do NOT name a category to do, do NOT present weekly session targets as things to do.
- Treat the health notes as possibly out of date: if they mention clearance for physical activity, IGNORE that clearance.
- You MAY acknowledge and respond warmly to activity she reports having done (walking, physiotherapy, prescribed movement).
- You MAY support sleep, nutrition, hydration and recovery.
- Defer every training decision to her medical team.` : '';
      const conditionsBlock = _activeC.length ? `

ACTIVE CIRCUMSTANCES (still true today, carried forward from when they were logged): ${_activeC.map(c=>`${c.text} (day ${daysElapsed(c, todayKey)+1})`).join("; ")}. Take these into account; do not treat them as new news each day.` : '';

      const todayDataCtx={
        recoveryBlock, conditionsBlock,
...
Last session: ${lastWorkoutType}, ${lastWorkoutDate?tripleLabel(lastWorkoutDate):"none on record"}
...
Last 14 days food log: ${last14FoodData.filter(d=>d.logged).map(d=>`${tripleLabel(d.date)} ${d.prot}g prot${d.meals.length?` (${d.meals.slice(0,3).join(", ")})`:""}` ).join(" | ")||"nothing logged"}
...
- PARTIAL REGENERATION: this is an update, not a fresh morning. Return the previous insights that the event does not touch UNCHANGED, word for word, and rewrite only the ones the event actually affects. Rewriting a still-true observation makes the card look unstable for no reason. If the event affects nothing, return the previous content unchanged.`;
        }
      }catch(e){}
      ...
      if(m){
        ...
        // A placeholder that is missing, OR one with a unit stuck to it, both
        // break rendering. Catch each at generation, not on screen.
        ...
        // Sensitivity enforcement on the main card too - the model is no more
        // reliable here than it was at the checkpoint. Drop, do not display.
        const _sens = profileData?.food_sensitivities || [];
        if(_sens.length){
          const before=(content.domain_insights||[]).length;
          content.domain_insights=(content.domain_insights||[]).filter(ins=>{
            const bad=violatesSensitivities(ins.content,_sens);
            if(bad.length) console.log("Coach: dropped an insight naming a food sensitivity ("+bad.join(", ")+"): "+String(ins.content).slice(0,80));
            return bad.length===0;
          });
          (content.domain_insights||[]).forEach(ins=>{
            if(ins.suggests_food) ins.suggests_food=ins.suggests_food.filter(f=>violatesSensitivities(f,_sens).length===0);
          });
          if(before!==content.domain_insights.length) content._sensFiltered=true;
        }
  ...
  // B.8 - LOGGING FOOD NO LONGER CALLS THE MODEL.
  // This used to fire a generation on every meal (~5-6/day) to restate what the
  // card could already work out. The invalidation engine now retires food advice
  // at render time from live data: instant, free, and it cannot fail offline.
  // Removal by default; the two daily checkpoints below are the only food-driven
  // generations left, and each is gated on something actually being off.

  async function runFoodCheckpoint(kind){
    if(IS_DEMO || !apiKey) return;
    const todayKey = new Date().toLocaleDateString("en-CA",{timeZone:getTz()});
    const doneKey = "coach_checkpoint_"+todayKey;
    let done={}; try{ done=JSON.parse(localStorage.getItem(doneKey)||"{}"); }catch(e){}
    if(done[kind]) return;
    const m = liveMetrics({allFood, fitbitData, profileData, cycleLog});
    const tgt = m.protein_target;
    const _cpHour = kind==="midday"
      ? Math.min(15.5, usualMealHour(allFood,"lunch")+0.75)
      : Math.max(Math.min(15.5, usualMealHour(allFood,"lunch")+0.75)+2, usualMealHour(allFood,"dinner")-2);
    const gate = checkpointGate(kind, m, tgt, {typicalByNow: typicallyEatenBy(allFood, _cpHour)});
    // Mark the checkpoint taken either way - one look per day, whether or not
    // it had anything to say. Otherwise a quiet gate re-evaluates every 5 min.
    done[kind]=true; try{ localStorage.setItem(doneKey, JSON.stringify(done)); }catch(e){}
    if(!gate.say){ console.log("Coach checkpoint ("+kind+"): on track, staying quiet."); return; }
    const eaten = (allFood[todayKey]||[]).map(e=>e.n).filter(Boolean).join(", ") || "nothing logged yet";
    // C.3 - trimmed context. This task needs six numbers, not her whole profile.
    // No "reply null" escape hatch: measured at 0/36 across both models, it was
    // decorative. The gate above has already decided this is worth saying.
    const sys = `You are Julia's nutrition coach, checking in during the day. Reply with ONE short sentence.
TONE: tentative and warm, never scolding. She may well have eaten without logging it - never assert that she has not eaten, and never imply she has failed at anything.
Write her running protein as the placeholder {protein_today} - never a literal number, and never followed by a unit. The app substitutes the live value and adds the unit itself, so "{protein_today}g" renders wrongly. Write "{protein_today}", not "{protein_today}g".`;
    const usr = `Time of day: ${kind==="midday"?"early afternoon":"late afternoon, a few hours before her usual dinner"}.
Logged so far (never suggest any of these again): ${eaten}
Protein logged: ${Math.round(m.protein_today)}g against a ${tgt}g target. Meals logged: ${m.meals_today}.
Why this check-in opened: ${gate.reason}.
${(profileData?.food_sensitivities||[]).length ? "NEVER suggest anything conflicting with: "+profileData.food_sensitivities.join(", ")+"." : ""}
Suggest at most one concrete, easy thing she could still eat today.`;
    const _sens = profileData?.food_sensitivities || [];
    let txt="";
    try{
      for(let attempt=0; attempt<2; attempt++){
        const extra = attempt===0 ? "" :
          "\n\nYour previous reply named something she cannot eat. Suggest something else entirely.";
        const raw = await callCoachAI(usr+extra, sys, {model:MODEL_FAST, maxTokens:120, cache:false});
        const cand = raw.replace(/```/g,"").trim();
        const bad = violatesSensitivities(cand, _sens);
        if(!bad.length){ txt=cand; break; }
        console.log("Coach checkpoint ("+kind+"): reply named a sensitivity ("+bad.join(", ")+") - "+(attempt===0?"retrying once.":"dropping it."));
      }
      // Saying nothing is always safe. Saying something she cannot eat is not.
      if(!txt){ return; }
      if(hasDoubleUnit(txt)){
        // Cheaper to repair than to regenerate, and the render guard would catch
        // it anyway; strip the stray unit so both layers agree.
        txt = txt.replace(/(\{[a-z_]+\})\s*(?:g|kcal|cal|grams|kilocalories|steps)\b/g, "$1");
        console.log("Coach checkpoint ("+kind+"): stripped a unit stuck to a placeholder.");
      }
      const ins = {type:"food_pattern", content:txt, claim:txt.slice(0,80),
        depends_on:{metric:"protein_today", value:Math.round(m.protein_today), threshold:tgt, direction:"gte", resolved_text:"Protein target hit ✓"}};
      setCoachContent(prev=>{
        const base = prev || {domain_insights:[]};
        const next = {...base,
      ...
      console.log("Coach checkpoint ("+kind+") fired: "+gate.reason);
    }catch(e){ console.log("Checkpoint error:", e.message); }
  }
  ...
  function shouldShowWeeklyReview() {
    ...
    const hour = parseInt(now.toLocaleString("en-CA",{timeZone:getTz(),hour:"numeric",hour12:false}));
    // CATCH-UP, not a window. This used to require the app to be open during
    // Saturday 18:00-23:59 -- about 3.5% of the week -- and a missed Saturday
    // was lost for ever, because the next check was already a new week. It went
    // five weeks without firing and nothing said so.
    //
    // The question is now "is there an unreviewed completed week", which is
    // answerable at any moment, on any day.
    const stored = weeklyReview;
    if(stored?.dismissed) return false;
    const weekStart = getWeekStartDate();
    if(stored?.generatedAt && new Date(stored.generatedAt) >= weekStart) return true;
    // The current week counts as reviewable once it has ended (Saturday
    // evening); any EARLIER week that was never reviewed is overdue right now.
    const isEndOfWeek = dow===6 && hour>=18;
    const missedAnEarlierWeek = !stored?.generatedAt || new Date(stored.generatedAt) < weekStart;
    return isEndOfWeek || missedAnEarlierWeek;
  }
...
🎯 THIS WEEK: <one sentence>`;
  ...
  // B.7 - two checkpoints a day, anchored to her own meal times. The gate is
  // evaluated locally every 5 min; a model call happens only if it opens.
  useEffect(()=>{
    if(!apiKey || !profileData || !fitbitReady || IS_DEMO) return;
    const check=()=>{
      const now=new Date();
      const hNow = parseInt(now.toLocaleString("en-US",{timeZone:getTz(),hour:"numeric",hour12:false}),10)
                 + parseInt(now.toLocaleString("en-US",{timeZone:getTz(),minute:"numeric"}),10)/60;
      const midday = Math.min(15.5, usualMealHour(allFood,"lunch")+0.75);
      const aft    = Math.max(midday+2, usualMealHour(allFood,"dinner")-2);
      if(hNow>=midday && hNow<aft) runFoodCheckpoint("midday");
      else if(hNow>=aft && hNow<22) runFoodCheckpoint("afternoon");
    };
    check();
    const iv=setInterval(check,5*60000);
    return ()=>clearInterval(iv);
  },[apiKey, fitbitReady, profileData?.uid, allFood]);
            ...
            return (
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {loading_ ? (
                  ...
                  <div key={ins.type+i} style={{display:"flex",alignItems:"flex-start",gap:8}}>
                    ...
                    <button onClick={()=>dismissInsight(ins.type)} title="Dismiss this for today"
                      style={{background:"none",border:"none",color:C.t3,fontSize:15,lineHeight:1,cursor:"pointer",padding:"2px 2px",flexShrink:0}}>×</button>
                  </div>
                ...
                {/* B.5 - manual refresh. Rate limited because each press costs a
                    real call, and the honest reason for refusing is worth saying. */}
                {apiKey&&!IS_DEMO&&(
                  <div style={{marginTop:2}}>
                    <button onClick={manualRegenerate} disabled={coachLoading}
                      style={{background:"none",border:"none",padding:0,fontSize:11,color:coachLoading?C.t3:C.pu,cursor:coachLoading?"default":"pointer"}}>
                      {coachLoading?"Refreshing…":"↻ Refresh"}
                    </button>
                    {regenMsg&&<span style={{fontSize:11,color:C.t3,marginLeft:8}}>{regenMsg}</span>}
                  </div>
                )}
              </div>
            );
...
function TabFood({allFood, setAllFood, protTgt, apiKey, onFoodLogged, suppState={}, setSupp, profileData, onSaveSupps, onSaveSensitivities, onSaveCalorieTarget}) {
  ...
  const [dtEditIdx, setDtEditIdx] = useState(null); // meal-header date/time editor (separate from ingredient editing)
  const [dtDate, setDtDate] = useState("");
  const [dtTime, setDtTime] = useState("");

  // Pure metadata edit: change WHEN a meal happened. No re-parsing, no AI call.
  // Same-day time edits re-sort in place; date edits move the meal to that day.
  // All date handling is on YYYY-MM-DD strings from the inputs — no Date math.
  async function saveDateTime(i){
    const entry=food[i];
    const newDate=dtDate, newTime=dtTime||entry.eaten_time||entry.time||"12:00";
    if(!newDate){ setDtEditIdx(null); return; }
    const updated={...entry, time:newTime, eaten_time:newTime};
    if(entry.dbid){
      try{ await supa("PATCH","food_log",{log_date:newDate,meal_time:newTime,eaten_time:newTime},"id=eq."+entry.dbid); }
      catch(e){ try{ await supa("PATCH","food_log",{log_date:newDate},"id=eq."+entry.dbid); }catch(e2){} }
    }
    setAllFood(prev=>{
      let next;
      if(newDate===foodDate){
        // `food` is a sorted copy but holds the same object references
        next={...prev,[foodDate]:(prev[foodDate]||[]).map(x=>x===entry?updated:x)};
      } else {
        next={...prev,
          [foodDate]:(prev[foodDate]||[]).filter(x=>x!==entry),
  ...
  // ── FREQUENT FOODS: automatic promotion/demotion ──────────────────────────
  // Promote: logged >=2x in trailing 30 days. Demote: not logged for 14 days
  // (deliberately generous — batch-cooked food is eaten across a week).
  // Manual X: takes effect immediately, suppresses re-promotion for 30 days.
  // Order: frequency, then recency. Cap 12.
  const [dismissedFoods, setDismissedFoods] = useState(()=>{
    if(profileData?.dismissed_foods) return profileData.dismissed_foods;
    try{return JSON.parse(localStorage.getItem("dismissed_foods")||"{}");}catch{return {};}
  });

  // Normalised key: same food at different quantities counts as one item.
  // Strips parentheticals, quantity+unit pairs, and bare numbers from the name.
  const normFoodKey=(n)=>(n||"").toLowerCase().trim()
    .replace(/\s*\([^)]*\)\s*/g," ")
    .replace(/\b\d+([.,]\d+)?\s*(g|gr|grams?|kg|ml|l|oz|pieces?|pcs|slices?|tbsp|tsp|cups?)\b/g," ")
    .replace(/\b\d+([.,]\d+)?\b/g," ")
    .replace(/\s+/g," ").trim();

  async function dismissFood(key){
    const next={...dismissedFoods,[key]:new Date().toISOString()};
    setDismissedFoods(next);
    try{localStorage.setItem("dismissed_foods",JSON.stringify(next));}catch{}
    try{await supa("POST","profiles",{uid:UID,dismissed_foods:next},"on_conflict=uid");}catch(e){/* column may not exist yet — localStorage carries it */}
  }

  const quickChips=(()=>{
    const todayK=tkey();
    const cutoff30=tkey(new Date(Date.now()-30*864e5));
    const cutoff14=tkey(new Date(Date.now()-14*864e5));
    const stats={}; // key -> {count,last,entry}
    Object.entries(allFood).forEach(([dk,list])=>{
      if(dk<cutoff30||dk>todayK) return;
      (list||[]).forEach(e=>{
        if(!e.n) return;
        const k=normFoodKey(e.n); if(!k) return;
        const st=stats[k]||(stats[k]={count:0,last:"",entry:null});
        st.count++;
        if(dk>=st.last){st.last=dk;st.entry=e;}
      });
    });
    const pinnedKeys=new Set(pinned.map(p=>normFoodKey(p.n)));
    return Object.entries(stats)
      .filter(([k,st])=>st.count>=2 && st.last>=cutoff14 && !pinnedKeys.has(k))
      .filter(([k])=>{
        const d=dismissedFoods[k];
        return !d || (Date.now()-new Date(d).getTime())>30*864e5; // 30-day suppression
      })
      .sort((a,b)=>b[1].count-a[1].count||b[1].last.localeCompare(a[1].last))
      .slice(0,12)
      .map(([k,st])=>({key:k,e:st.entry}));
  })();
...
Input: ${txtInput}`;
      ...
      setAllFood(prev=>{
        ...
        _dayAfter = updated[targetDate];
        localStorage.setItem("jfood_backup", JSON.stringify(updated));
        return updated;
      });
      // B.4 - the confirmation carries the running total, computed locally.
      setAiMsg(saveErr
        ? entry.n + " saved locally only — DB error: "+saveErr.slice(0,60)
    ...
    <div>
      ...
      {(quickChips.length>0||pinned.length>0||(allFood[tkey(new Date(Date.now()-864e5))]||[]).length>0)&&(
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:6,marginBottom:8,WebkitOverflowScrolling:"touch"}}>
          ...
          {quickChips.map(({key,e})=>(
            <span key={"chip"+key} style={{display:"inline-flex",alignItems:"center",flexShrink:0,background:C.sf,border:`.5px solid ${C.bd}`,borderRadius:8,overflow:"hidden"}}>
              <button onClick={()=>cloneEntryTo(e,foodDate)} style={{background:"none",border:"none",padding:"5px 4px 5px 12px",fontSize:11,color:C.t2,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>{e.n}</button>
              <button onClick={()=>dismissFood(key)} title="Remove from quick-add (30 days)" style={{background:"none",border:"none",padding:"5px 8px 5px 4px",fontSize:12,color:C.t3,cursor:"pointer",lineHeight:1}}>×</button>
            </span>
          ))}
        </div>
      )}
          ...
          <div key={i} style={{background:C.sf,borderRadius:8,border:`.5px solid ${C.bd}`,marginBottom:6,overflow:"hidden"}}>
            <div style={{display:"flex",alignItems:"center",padding:"10px 14px"}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                {/* WHEN control — separate from ingredient editing, pure metadata */}
                {dtEditIdx===i?(
                  <div style={{display:"flex",alignItems:"center",gap:4,marginTop:4,flexWrap:"wrap"}}>
                    <input type="date" value={dtDate} onChange={ev=>setDtDate(ev.target.value)} style={{...s.input,width:128,padding:"3px 6px",fontSize:11}}/>
                    <input type="time" value={dtTime} onChange={ev=>setDtTime(ev.target.value)} style={{...s.input,width:84,padding:"3px 6px",fontSize:11}}/>
                    <button onClick={()=>saveDateTime(i)} style={{...s.btn("p"),padding:"3px 9px",fontSize:11}}>✓</button>
                    <button onClick={()=>setDtEditIdx(null)} style={{background:"none",border:"none",color:C.t3,cursor:"pointer",fontSize:13}}>×</button>
                  </div>
                ):(
                  <button onClick={()=>{setDtEditIdx(i);setDtDate(foodDate);setDtTime(e.eaten_time||e.time||"");}} title="Edit date & time"
                    style={{background:"none",border:"none",padding:0,marginTop:2,fontSize:10,color:C.t3,cursor:"pointer",fontFamily:"inherit",textDecoration:"underline dotted",textUnderlineOffset:2}}>
                    🕐 {e.time||"set time"}
                  </button>
                )}
              </div>
...
// ── CYCLE TAB ─────────────────────────────────────────────────────────────
// ── SYMPTOM TAXONOMY ────────────────────────────────────────────────────────
// Curated down from Flo's ~60: cycle tracking is one tab here, not the product,
// and a long list produces no logging at all. Anything omitted is reachable
...
function TabCycle({cycleDates, setCycleDates, cycleLog, setCycleLog}) {
  ...
  function persistDraft(d){
    ...
    symTimer.current=setTimeout(()=>{
      ...
      if(draftEmpty(d)) delete next[symDate]; else next[symDate]=cleanDraft(d);
      saveCycleLog({symptom_logs:next});
    },450);
  }
  function closeSymptom(){
    if(symTimer.current) clearTimeout(symTimer.current);
    const next={...symptomLogs};
    if(draftEmpty(symDraft)) delete next[symDate]; else next[symDate]=cleanDraft(symDraft);
    saveCycleLog({symptom_logs:next});
  ...
  return (
    <div>
      ...
      {st.cycleDay&&(
        <Card>
          ...
          {/* Discharge lives here, not in symptoms — it is how most people
              actually recognise ovulation. */}
          <div style={{marginTop:12,paddingTop:12,borderTop:`.5px solid ${C.bd}`}}>
            <div style={{fontSize:11,fontWeight:600,color:C.t2,marginBottom:7}}>Cervical fluid today <span style={{fontWeight:400,color:C.t3}}>(optional)</span></div>
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              {DISCHARGE_TYPES.map(x=>{
                const on=(symptomLogs[tkey()]||{}).discharge===x;
                return <button key={x} onClick={()=>{
                  const cur=symptomLogs[tkey()]||{};
                  const next={...symptomLogs,[tkey()]:{...cur,discharge:cur.discharge===x?null:x}};
                  saveCycleLog({symptom_logs:next});
                }} style={{fontFamily:"inherit",fontSize:12.5,fontWeight:500,padding:"7px 13px",borderRadius:20,cursor:"pointer",
                  background:on?C.pi:C.s2,color:on?"#fff":C.t2,border:`1px solid ${on?C.pi:C.bd}`}}>{x}</button>;
              })}
            </div>
          </div>
          <button onClick={()=>setOvHelp(v=>!v)} style={{background:"none",border:"none",padding:0,marginTop:12,fontSize:11,color:C.pu,cursor:"pointer",fontWeight:500}}>{ovHelp?"▾":"▸"} How do I know if I'm ovulating?</button>
          {ovHelp&&(
      ...
      {showSymptom&&(()=>{
        ...
        const Chips=({list,sel,onTap})=>(
          <div style={row}>{list.map(x=>(
        ...
        <div style={s.mo} onClick={e=>{if(e.target===e.currentTarget)closeSymptom();}}>
          <div style={{...s.modal,maxHeight:"90vh",overflowY:"auto"}}>
            <h3 style={{fontSize:16,fontWeight:600,marginBottom:10}}>How are you today?</h3>
...
// Structured Life Context taps — these ARE the confounder layer the cycle
// rebuild checks before attributing any pattern to phase. Free text cannot be
// machine-checked, so these are stored as canonical tokens ahead of any note:
...
function contextTagsOf(entry){
  if(!entry||!entry.txt) return [];
  const t=displayTag(entry.tag);
  if(t!=="life_context") return [];
  const head=entry.txt.split(CONTEXT_SEP)[0]||"";
  return CONTEXT_TAGS.filter(tag=>head.toLowerCase().includes(tag.toLowerCase()));
}
...
function CoachMemoryCard({profileData, fitbitData, apiKey}) {
  ...
  // Refresh when STALE, not only when absent. It was generated once and never
  // again, so "what your coach knows about you" was a July snapshot -- actively
  // misleading now that it sits on Today.
  const MEMORY_MAX_AGE_DAYS = 14;
  React.useEffect(()=>{
    if(!apiKey||IS_DEMO) return;
    const ts = profileData?.coach_memory_generated_at;
    const ageDays = ts ? (Date.now()-new Date(ts).getTime())/864e5 : Infinity;
    if(!memory || ageDays > MEMORY_MAX_AGE_DAYS) generate();
  },[apiKey, profileData?.coach_memory_generated_at]);
...
export default function App() {
  ...
  // One table on load, so a schedule that stops is visible rather than silent.
  const _reportedSched = React.useRef(false);
  useEffect(()=>{
    if(!profileData || _reportedSched.current) return;
    _reportedSched.current = true;
    reportScheduledArtifacts(profileData);
  },[profileData?.uid]);

  // Phase 5.4 migration — runs once, then never again.
  useEffect(()=>{
    if(IS_DEMO||!profileData||!logEntries||!logEntries.length) return;
    let done=false; try{ done=!!localStorage.getItem(MIGRATION_KEY); }catch(e){}
    if(done) return;
    const built = buildConditionsFromLog(logEntries, profileData.conditions);
    try{ localStorage.setItem(MIGRATION_KEY,"1"); }catch(e){}
    if(!built.length) return;
    const next=[...(profileData.conditions||[]), ...built];
    const openCount=built.filter(c=>!c.resolved_at).length;
    console.log(`Phase 5.4 migration: ${built.length} log entr${built.length===1?"y":"ies"} became conditions (${openCount} still active).`);
    setProfileData(p=>({...p, conditions:next}));
    supa("POST","profiles",{uid:UID,conditions:next},"on_conflict=uid")
      .catch(e=>console.log("Migration save failed:", e.message));
  },[profileData?.uid, logEntries.length]);
  ...
  const TABS=[{id:"dash",label:"Today"},{id:"food",label:"Food"},{id:"log",label:"Log"},{id:"cycle",label:"Cycle"},{id:"progress",label:"Progress"}];
  ...
  return (
    <div style={s.shell}>
      ...
      <nav className="bottomNav">
        {[["dash","home","Today"],["food","food","Food"],["log","log","Log"],["cycle","moon","Cycle"],["progress","progress","Progress"]].map(([id,icon,label])=>(
