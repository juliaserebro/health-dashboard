...
function TabDash({allFood, logEntries, cycleDates, apiKey, protTgt, aiRefreshTick=0, fitbitData={sleep:[],steps:[],workouts:[]}}) {
  ...
  async function genAIInner(type) {
    ...
    if(confDates.length){const last=new Date(confDates[0].d);const day=Math.floor((now-last)/864e5)+1;const cd=((day-1)%28)+1;cyclePhaseName=cd<=5?"menstrual":cd<=13?"follicular":cd<=16?"ovulatory":"luteal";cyclePhase=`cycle day ${cd}/28, ${cyclePhaseName} phase`;}
    // Today nutrition
    const todayFoodKey = now.toISOString().slice(0,10);
    ...
    // This week food per day
    const weekFoodLines=[];
    for(let i=0;i<=dowIL;i++){const d=new Date(now.getFullYear(),now.getMonth(),now.getDate()-dowIL+i);const dk=d.toISOString().slice(0,10);const m=allFood[dk]||[];if(m.length){const p=Math.round(m.reduce((s,e)=>s+(e.p||0),0));const c=Math.round(m.reduce((s,e)=>s+(e.c||0),0));const k=Math.round(m.reduce((s,e)=>s+(e.k||0),0));weekFoodLines.push(`${d.toLocaleDateString("en-GB",{weekday:"short"})}: ${p}g prot ${c}g carb ${k}kcal`);}}
...
DATA: Last night sleep: ${sleepSummary}. Yesterday alcohol: ${yAlc||"none"}. Today's workout: ${todayActivity}. Today food: ${liveProt}g protein (target ${protTgt}g), ${liveCarbs}g carbs, ${liveKcal}kcal.
TASK: Write 2–3 short insights that connect dots between her data. Focus on HOW things relate — e.g. does her sleep connect to something from yesterday? Does her cycle phase explain her appetite or energy? Does a logged mood/pain link to sleep or recovery? Only draw connections that the data actually supports — do not guess. Language: warm, forgiving, no judgment.
FORMAT: 2–3 sections. Each = one emoji + CAPS LABEL: **one bold key point.** One sentence max explaining the connection. Total max 80 words. ${isEvening?"Last section = TONIGHT (wind-down/sleep prep for her cycle phase).":""}No generic advice. Cycle phase MUST appear.`
...
This week's food: ${weekFoodSummary}. Sleep this week: ${weekSleep||"limited data"}.
TASK: Write 2–3 short insights connecting her data across this week. Look for patterns: does her eating pattern fit her cycle phase? Does her sleep correlate with activity or food? Any trend worth noting? Do NOT state obvious targets, do NOT recommend specific exercises or workout types. Language: warm, forgiving.
FORMAT: 2–3 bullet points, one sentence each. Total max 60 words. Cycle phase MUST be mentioned by name.`;
    try {
