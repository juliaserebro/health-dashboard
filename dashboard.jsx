...
function handleGoogleCallback(){
  ...
  if(error){
    // Silent re-auth failed (prompt:none) — fall back to visible consent
  ...
  window.history.replaceState(null,"",window.location.pathname);
  // Persist token to Supabase so it survives across devices (fire-and-forget)
...
async function ghFullSync(setSyncStatus,setFitbitData){
  ...
  try{
    ...
    try{
      ...
      const rhrMap={};
      console.log("RHR raw sample:", JSON.stringify((rhrData.dataPoints||[]).slice(0,2)));
      (rhrData.dataPoints||[]).forEach(pt=>{
        ...
        if(date&&bpm!=null) rhrMap[date]=Math.round(parseFloat(bpm));
      });
...
Rules: no section repeats another; all language warm and non-guilt; never mention specific muscle groups or body parts.`;
  ...
  async function refreshNutritionOnly() {
    ...
    // Gate: only call AI for meaningful food events
    const prevHash = (()=>{try{return JSON.parse(localStorage.getItem("coach_content_"+todayKey)||"{}") ._foodHash||"";}catch{return "";}})();
    const prevCount = prevHash.split("|").filter(Boolean).length;
    const isFirstMeal = prevCount===0 && todayFoodEntries.length>0;
    const prevProt = parseInt(localStorage.getItem("coach_last_protein_"+todayKey)||"0");
    const proteinJustCrossed = prevProt < protTgt2 && prot >= protTgt2;
    const insightHistory = (()=>{try{return JSON.parse(localStorage.getItem("coach_insight_history")||"{}");}catch{return {};}})();
    const lastNutritionMs = insightHistory.food_pattern?.last_surfaced_ms ? new Date(insightHistory.food_pattern.last_surfaced_ms) : null;
    const hoursSinceNutrition = lastNutritionMs ? (now-lastNutritionMs)/3600000 : 999;
    if(!isFirstMeal && !proteinJustCrossed && hoursSinceNutrition<4){
      // Update hash + protein counter without calling AI
      try{const c=JSON.parse(localStorage.getItem("coach_content_"+todayKey)||"{}");c._foodHash=currentFoodHash;localStorage.setItem("coach_content_"+todayKey,JSON.stringify(c));setCoachContent(prev=>prev?{...prev,_foodHash:currentFoodHash}:prev);}catch(e){}
...
export default function App() {
  ...
  useEffect(()=>{
    // Handle Google OAuth callback (token or error in URL hash)
    ...
    async function load(){
      ...
      try{
        ...
        if (prof) {
          ...
          if(prof.google_access_token && prof.google_token_expiry) {
  ...
  async function syncGoogleHealth() {
    if(!isGTokenValid(getGToken())) {
      // Token expired — try silent re-auth first (instant if Google session still active)
      setSyncStatus("Reconnecting to Google Health...");
      startGoogleAuth(false); // uses prompt:none if previously consented
      return;
    }
    setGhSyncing(true);
    try {
